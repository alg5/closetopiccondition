...
class moder extends \phpbb\notification\type\base
{
	...
	*/
	...
	{
		return 'alg.closetopiccondition.notification.type.moder';
	}
	...
	* @var string
	*/
	protected $language_key = 'NOTIFICATION_MODER';
	/**
	...
	*/
	...
	{
		//return append_sid($this->phpbb_root_path . 'viewforum.' . $this->php_ext, "f={$this->get_data['forum_id']}");
		return '';
	}
	...
	public function create_update_array($data)
	{
		$sql = 'SELECT notification_data
			...
			WHERE notification_type_id = ' . (int) $this->notification_type_id . '
				AND item_id = ' . (int) self::get_item_id($data);
		$result = $this->db->sql_query($sql);
		if ($row = $this->db->sql_fetchrow($result))
		{
			$data = unserialize($row['notification_data']);
		}
		return $this->create_insert_array($commandgame_data);
	}        
